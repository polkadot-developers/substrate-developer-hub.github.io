...
* `--alice` Puts the pre-defined Alice keys (both for block production and finalization) in the node's keystore. Generally one should generate their own keys and insert them with an RPC call. We'll generate our own keys in a later step.
...
`Initializing Genesis block/state (state: 0x4faf…3aba, header-hash: 0x8855…e564)` tells which genesis block the node is using. Bob's node must have the same hashes or they will not connect to one another.
...
* If these two nodes are running on the same physical machine, Bob MUST specify a different `--base-path`, `--port`, `--ws-port`, and `--rpc-port`.
