...
3. Each benchmark case has three sections, setup section, execution section, and optionally a
   verification section at the end.
    ...
    The name of the benchmark case is `set_dummy_benchmark`. Here `b` is the input that passed into
    the extrinsic `set_dummy`. `b` will be varied from 1 to 1,000 and run repeatedly for each value
    of `b` according to the arguments we passed when running the benchmark command. We will get back
...
functions of benchmarked extrinsics with the computed coefficient above. You can directly integrated
...
To use this file, you first define a `WeightInfo` trait in
...
The generated file implements these weight functions. To integrate them, we can copy the generated
file into the directory where `lib.rs` is located and named it as `weights.rs`.
